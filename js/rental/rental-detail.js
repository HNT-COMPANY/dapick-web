...
// 표에 없는 키가 와도 개월 표기가 그대로 새어 나가지 않게 한다.
// 관리자가 '의무48/계약48' 같은 조합을 새로 만들면 표에는 없다 — 그때도 4년 계약으로 읽혀야 한다.
// 12로 안 떨어지면(예: 계약30) 년으로 못 바꾸므로 원래 키를 둔다. 틀린 글자보다 낫다.
function contractLabel(key) {
  if (RD_CONTRACT_LABELS[key]) return RD_CONTRACT_LABELS[key];
  const m = String(key || '').match(/계약(\d+)/);
  if (!m) return key;
  const y = Number(m[1]) / 12;
  return y && y === Math.floor(y) ? y + '년 계약' : key;
}
